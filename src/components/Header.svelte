...
>
	<h1 class="font-medium">
		<b class="font-bold poppins">Amay</b> Lunagaria
	</h1>
	<div class="flex ml-auto pr-4 items-center gap-4">
		{#each tabs as tab, index}
			<a href={tab.link} class="duration-200 hover:text-violet-400 hidden md:inline-block" target={index === 2 ? '_blank' : ''}>
				<p>{tab.name}</p>
			</a>
		{/each}
		<a href="#check" target="_blank" class="blueShadow relative overflow-hidden px-5 py-2 group rounded-full bg-white text-slate-950">
